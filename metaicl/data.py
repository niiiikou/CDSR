...
class MetaICLData(object):
    ...
    def __init__(self, logger=None, gpt="gpt2-large", method="channel", 
        ...
        if gpt.startswith('gpt3'):
            ...
            if self.n_prefix_tokens>0:
                ...
                # Print the first few entries in the vocabulary
                # print(self.model.get_input_embeddings().weight)
                # print("token_id:", list(vocabulary.items())[50267])

                # for token, token_id in list(vocabulary.items()):
                #     if token_id in [50267, 50268, 50269, 50270, 50271, 50272, 50273, 50274, 50275, 50276,23318]:
    ...
    def _prepro_each_datapoint(self, dp, is_first=True, is_training=False, for_demonstrations=False):
        ...
        if is_training or for_demonstrations:
            ...
            # print("option_tokens:",option_tokens)
    ...
    def _tensorize_for_training(self, train_data):
        ...
        n_answers = []
        print("self.use_demonstrations:",self.use_demonstrations)
        ...
        if self.use_demonstrations:
            ...
            for dp in train_data:
                ...
                encoded = prepro_sentence_pair_single(
                    ...
                    self.prefix_token_ids, allow_truncation=True, 
                    task=task if self.task_counts is not None else None)
                # print("input_ids:",encoded[0])
                # print("attention_mask:",encoded[1])
                # print("token_type_ids:",encoded[2])
                input_ids.append(encoded[0])
                attention_mask.append(encoded[1])
                token_type_ids.append(encoded[2])
    ...
    def tensorize(self, _train_data, _test_data, instruction=None, options=None):
        # in prior_test _train_data= None
        input_ids, attention_mask, token_type_ids = [], [], []
        metadata = []
        ...
        # in prior_test use_demonstrations:=False
        if self.use_demonstrations:
    ...
    def tensorize_for_training(self, train_data, keyword, seed, use_random_english_words=False):
        ...
        print("use_random_english_words",use_random_english_words)
        ...
        # print("inputs[input_ids]", inputs["input_ids"][0:10,0:10])
        # print("inputs[attention_mask]", inputs["attention_mask"][0:10,0:10])
        # print("inputs[token_type_ids]", inputs["token_type_ids"][0:10,0:10])
